...
<html lang="en">
	<head>
		<title>Latihan Soal | Lembaga Pendidikan Islam Hidayatullah</title>
...
<!--	    <link rel="stylesheet" type="text/css" href="<?php echo base_url('assets/pg_user/css/jPaginator.css');?>">-->
	...
	<body onload="JavaScript:connect();" class="fullpage-latihansoal">
		<div class="header">
			<!-- Navbar  -->
	...
	<?php include "modal/modal_pembahasan.php"; ?>
		...
		<div class="page-body latihan-soal">
			<div class="soal-header">
					<h2><?php echo $data->nama_sub_materi ? $data->nama_sub_materi : '' ?></h2>
			</div>

			<?php
			// backup kondisi ($header->status_materi == 0) OR ($allow_akses == true)
		if($kondisi_flag==0)
			{ ?>
						...
						<form id="form_soal" method="post" action="<?php echo base_url('latihan/penilaian')?>">
							...
							{ ?>
								<div id="item_soal_<?php echo $no;?>" class="tab-pane fade
									<?php if($no==1) {?> active in<?php } ?>
								">
									<div class="row equal-row item-soal">

										<div class="col-sm-6 equal-col soal">

											<div class="keterangan-wrapper">
												<?php $jml_soal = count($data_soal); ?>
												<ul class="keterangan">
													<li>Soal <span id="soal_number"><?= $no ?></span> dari <b><?php echo $jml_soal?></b></li>
										...
										<div class="col-sm-6 equal-col jawaban">
											<h5><b>Pilihan jawaban</b></h5>
											<input type="radio" class="pilihan_jawaban" name="pilihan_jawaban<?php echo $no;?>" id="opt-a_<?php echo $no;?>" value="<?php echo $item->id_soal; ?>-1" id-soal=<?php echo $item->id_soal;?>>
											...
											<input type="radio" class="pilihan_jawaban" name="pilihan_jawaban<?php echo $no;?>" id="opt-b_<?php echo $no;?>" value="<?php echo $item->id_soal; ?>-2" id-soal=<?php echo $item->id_soal;?>>
											...
											<input type="radio" class="pilihan_jawaban" name="pilihan_jawaban<?php echo $no;?>" id="opt-c_<?php echo $no;?>" value="<?php echo $item->id_soal; ?>-3" id-soal=<?php echo $item->id_soal;?>>
											...
											<input type="radio" class="pilihan_jawaban" name="pilihan_jawaban<?php echo $no;?>" id="opt-d_<?php echo $no;?>" value="<?php echo $item->id_soal; ?>-4" id-soal=<?php echo $item->id_soal;?>>
											...
											{ ?>
											<input type="radio" class="pilihan_jawaban" name="pilihan_jawaban<?php echo $no;?>" id="opt-e_<?php echo $no;?>" value="<?php echo $item->id_soal; ?>-5" id-soal=<?php echo $item->id_soal;?>>
												...
												<div class="col-md-12">
													...
													?>
														<div class="btn-group btn-bahas">
															...
															<a data-toggle="modal" data-target="#pembahasanModal" class="modal-pembahasan btn btn-turquoise btnbahasteks hidden" data-action="<?php echo $item->id_soal;?>-teks"><span class="glyphicon glyphicon-file"></span> <span class="teks">Teks</span></a>
															<?php
															}

															if(!empty($item->pembahasan_video)) { ?>
															<a data-toggle="modal" data-target="#pembahasanVideoModal" class="modal-pembahasan btn btnbahasvid btn-redflamingo hidden" data-action="<?php echo $item->id_soal;?>-video"><i class="glyphicon glyphicon-play-circle"></i> <span class="teks">Video</span></a>
															<?php
															}
															?>
														</div>
							...
							<div class="soal-pagination">
								...
								<nav class="text-center">
									...
									{ ?>
										...
										<li class="<?php if($no==1){echo "active";}?>"><a data-toggle="tab" href="#item_soal_<?php echo $no;?>" id="toggle_item_soal_<?php echo $no;?>"><?php echo $no;?></a></li>
...
//		});
	...
	<script type="text/javascript">
		$(document).ready(function(){
			...
			$("input.pilihan_jawaban").click(function(e){
				idsoal = $(this).attr("id-soal");
				$("[data-action*='"+idsoal+"']").removeClass('hidden');
				$("[data-action*='"+idsoal+"']").addClass('show');
		...
		function ajaxCheckJawaban(action, id_opt)
		{
		  ...
		  // console.log("StatusCheckJawaban: " + status + "\nDATA: " + data);
					...
					if(data == "benar")
					{
						...
						$("#toggle_item_soal_" + id_opt[1]).html("<i class='glyphicon glyphicon-ok'style='color:#1bbc9b;'></i>");
						$('#correct').modal('show');
						setTimeout(function(){
					...
					else if(data == "salah")
					{
						...
						$("#toggle_item_soal_" + id_opt[1]).html("<i class='glyphicon glyphicon-remove' style='color:#dc1826;'></i>");
						$('#wrong').modal('show');
						setTimeout(function(){
