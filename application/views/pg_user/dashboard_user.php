...
function loaderimg()
{
    echo '<svg xmlns="http://www.w3.org/2000/svg" width="200px" height="200px" viewBox="0 0 100 100" preserveAspectRatio="xMidYMid" class="lds-progress" style="background: none;"> <defs> <clipPath ng-attr-id="{{config.cpid}}" x="0" y="0" width="100" height="100" id="lds-progress-cpid-dc33259843c9a"> <rect x="0" y="0" width="74.5193" height="100"> <animate attributeName="width" calcMode="linear" values="0;100;100" keyTimes="0;0.5;1" dur="1.2" begin="0s" repeatCount="indefinite"/> <animate attributeName="x" calcMode="linear" values="0;0;100" keyTimes="0;0.5;1" dur="1.2" begin="0s" repeatCount="indefinite"/> </rect> </clipPath> </defs> <path fill="none" ng-attr-stroke="{{config.c1}}" stroke-width="2.7928" d="M82,63H18c-7.2,0-13-5.8-13-13v0c0-7.2,5.8-13,13-13h64c7.2,0,13,5.8,13,13v0C95,57.2,89.2,63,82,63z" stroke="#16a085"/> <path ng-attr-fill="{{config.c2}}" ng-attr-clip-path="url(#{{config.cpid}})" d="M81.3,58.7H18.7c-4.8,0-8.7-3.9-8.7-8.7v0c0-4.8,3.9-8.7,8.7-8.7h62.7c4.8,0,8.7,3.9,8.7,8.7v0C90,54.8,86.1,58.7,81.3,58.7z" fill="#1abc9b" clip-path="url(#lds-progress-cpid-dc33259843c9a)"/> </svg>';
}
    ...
    <div class="col-lg-12">
		...
		<div class="agcu-welcome">
			<div class="content">
				...
				<p>Ketahui tipe kepribadian, kondisi psikologis, potensi akademik dan minat belajar anda dengan
					mengikuti Academic General Check Up (AGCU) Test. Dengan mengikuti AGCU test, anda akan mendapatkan
					saran metode belajar yang sesuai dengan tipe kepribadian yang anda miliki. </p>
				<?php
        if ($status_siswa == "tidak_aktif") {
            ?>
					<button type="button" class="btn btn-primary btn-lg btn-turquoise" data-toggle="modal" data-target="#aktivasiagcu">
						Mulai AGCU
					</button>
					<?php
        } else {
            ?>
					<a href="../agcutest" class="btn btn-primary btn-turquoise">Mulai AGCU</a>
					<?php
        }
        ?>
			</div>
...
-->
        ...
        foreach ($kelasaktif as $kelas) {
            ?>
            <div class="stat-wrapper" style="padding:0;" id="materi-home">
                <h1 style="text-transform:uppercase;">Materi <?php echo $kelas->alias_kelas; ?></h1>
                <input class='materi-home-btn btn btn-link pull-right hidden' value="<< Tutup"
                       btn-ke="<?php echo $kelas->id_kelas; ?>"></input>
            </div>

            <div class="mapel-wrapper text-center" id="materi">

                <div id="materi<?php echo $kelas->id_kelas; ?>">
                    ...
                    <?php
                    foreach ($carimapel[$kelas->id_kelas] as $mapel) {
                        echo "
									<div class='mapel-container'>
										<div class='content'>
											...
											<input type='hidden' id='m$mapel->id_mapel' value='$kelas->id_kelas'/>
        ...
        <!-- <div class="mapel-container">
			<div class="content">
				...
				<button class="btn btn-dashboard-materi btn-kelas" type="submit" id="<?php echo $kelas->id_kelas; ?>">Lihat Mata Pelajaran
				</button>
			</div>
		</div> -->
        ...
        <form action="statistiknilai" method="get">
            <div class="col-lg-12">
                ...
                <input class="btn btn-danger" id="submitstatistik" type="submit" value="Lihat Statistik"
    ...
    <div class="col-lg-12">
        ...
        <div class="akun-video">
            <div class="stat-wrapper" style="padding:0;">
                <div class="diagnistic-header">
                    <h1 style="text-transform:uppercase;padding:10px 50px;">BONUS VIDEO MOTIVASI &amp; INSPIRASI</h1>
                </div>
            </div>
            <div class="video-wrapper">
                <?php
                $no = 1;
                if (count($data_video_motivasi) > 0) {
                    foreach ($data_video_motivasi as $video) {
                        ...
                        ?>
                        <a href="#;" target="_BLANK" class="modal-video-motivasi"
                           data-value="<?php echo $video->id_konten ?>" data-toggle="modal" <?php echo $url ?> >

                            <div class="video-container">
                                <div class="video">
                                    <div class="caption">
                                        <img class="lock"
                                             src="<?php echo base_url('assets/dashboard/images/lock.png'); ?>"
        ...
        <div class="akun-video">
            <div class="stat-wrapper" style="padding:0;">
                <div class="diagnistic-header">
                    <h1 style="text-transform:uppercase;padding:10px 50px;">BONUS KONTEN BELAJAR</h1>
                </div>
            </div>
            <div class="video-wrapper">
                <?php
                $no = 1;
                if (count($data_bonus) > 0){
                foreach ($data_bonus
                ...
                <a href="<?php echo $url ?>" target="_BLANK"
                   data-value="<?php echo $bonus->id_konten ?>"
                   data-toggle="modal" <?php echo $target ?> >
                    <div class="video-container">
                        <div class="video">
                            <div class="caption">
                                <img class="lock" src="<?php echo base_url('assets/dashboard/images/lock.png'); ?>"
                                     style="<?php echo $style ?>">
...
foreach ($kelasaktif as $kelas) {
    foreach ($data_profil as $profil) {
        if ($profil->id_kelas == $kelas->id_kelas and $profil->status == 1) {
            ?>
            <div class="modal fade" id="modalcbtcontest<?php echo $profil->id_tryout; ?>" tabindex="-1" role="dialog"
                ...
                <div class="modal-dialog modal-dialog-contest" role="document">
                    <div class="modal-content modal-content-contest">
                        <div class="modal-body body-modal-contest">
                            <img src="<?php echo base_url('assets/uploads/banner/' . $profil->banner); ?>"
                            ...
                            <div class="row contest-desc">
                                ...
                                <div class="col-lg-5 col-md-5 col-sm-5">
                                    ...
                                    <a href="../cbt/cbt_detail/<?php echo $profil->id_tryout; ?>"
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $(".btn-kelas").click(function () {
            $("#materi").html("<img src='<?php echo base_url('assets/pg_user/images/gears.gif'); ?>' style='margin: 20px auto 0px; width: 200px;' />");
...
<!-- MATERI DASHBOARD 2017-08-08 -->

<script>
    $(function () {
        $(".btn-pilihmapel").click(function () {
            var kelas = $("#m" + $(this).attr('id')).val();
            $("#materi" + kelas).html("<img src='<?php echo base_url('assets/pg_user/images/gears.gif'); ?>' style='margin: 20px auto 0px; width: 200px;' />");
