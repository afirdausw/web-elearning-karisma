...
<script type="text/javascript">
    $(function () {
        $("#kelas").change(function () {
            ...
            var urlSearch = "<?php echo base_url() . 'psep_sekolah/materi/tabel_ajax/'; ?>" + $("#kelas").val() + "/0/";
            $.ajax({
                url: urlSearch,
                beforeSend: function () {
                    NProgress.start();
        ...
        $("#mapel").change(function () {
            var urlSearch = "<?php echo base_url() . 'psep_sekolah/materi/tabel_ajax/'; ?>" + $("#kelas").val() + "/" + $("#mapel").val() + "/0/";
            $.ajax({
                url: urlSearch,
                beforeSend: function () {
                    NProgress.start();
        ...
        $("#mapok").change(function () {
            var urlSearch = "<?php echo base_url() . 'psep_sekolah/materi/tabel_ajax/'; ?>" + $("#kelas").val() + "/" + $("#mapel").val() + "/" + $("#mapok").val();
            $.ajax({
                url: urlSearch,
                beforeSend: function () {
                    NProgress.start();
...
<div class="row" style="margin-bottom:20px;">
    ...
    <div class="col-md-3">
        <select id="mapel" class="form-control">
            ...
            <?php if ($idkelas > 0) { ?>
                <?php foreach ($carimapel as $mapel) { ?>
...
<div class="table-responsive">
    <table id="my_datatable" class="table table-bordered table-hover">
        <thead>
        <tr>
            <th>#</th>
            ...
            foreach ($data_tabel as $row) {
                if ($row->id_sub_materi != '') {
                    ...
                    <tr>
                        ...
                        <td class="text-center">
                            <a href="<?= base_url() ?>psep_sekolah/materi/manajemen/ubah?id=<?= $row->id_sub_materi ?>"
                               class="btn btn-warning btn-xs" title="Ubah"><span
                                        class="glyphicon glyphicon-pencil"></span></a>


                            <button type="button" class="btn btn-danger btn-xs" title="Hapus" data-number="<?= $i ?>"
                                    value="<?= $row->id_sub_materi ?>" data-toggle="modal"
                                    data-target="#deleteRow_modal"><i class="glyphicon glyphicon-trash"></i></button>
