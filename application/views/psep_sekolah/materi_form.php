					...
					<div class="col-md-12">
						<div class="card">
							<div class="header">
								...
								<p class="category"><?=(isset($jenjang)) ? $jenjang : " " ?></p>
							</div>
							<div class="content">
											...
											<div class="col-md-6">
												<select data-placeholder="Pilih Mata Pelajaran..." name="nama_mapel" class="chosen-select" onchange="fetch_select_materi_pokok(this.value)" style="width: 100%;" tabindex="2" required="required">
													...
													foreach ($select_options_mapel as $item) { 
														if($item->jenjang== $jenjang){
														$option_name = $item->nama_mapel . " - " . $item->alias_kelas;
														?>
														<option  <?php echo($item->id_mapel == $idmapel ? "selected" : "") ?> value="<?php echo $item->id_mapel ?>" > 
											...
											<div class="col-xs-6 col-md-6">
												<select data-placeholder="Pilih Materi Pokok..." id="materi_pokok" name="materi_pokok" class="chosen-select" style="width: 100%;" tabindex="3" required="required">
													...
													foreach ($select_options_materi_pokok as $item) {
														if($item->id_kelas == $idkelas && $item->id_mapel == $idmapel){
														$option_name = $item->nama_mapel . " - " . $item->alias_kelas;
														?>
																			...
																			<div class="col-md-6">
																				<div class="form-group">
																					...
																					<select data-placeholder="Kunci Jawaban" name="kunci_jawaban<?php echo $ke; ?>" class="form-control" >
																						...
																						<option <?php echo set_select('kunci_jawaban<?php echo $ke; ?>', '5', (!isset($data_soal->kunci_jawaban) ? FALSE : ($data_soal->kunci_jawaban == '5' ? TRUE : FALSE)) );?> value="4">Jawaban 5</option>
																					</select>
																				</div>
																			</div>
																		</div>
														...
														<div class="row">
															<div class="col-md-6">
																...
																<select data-placeholder="Kunci Jawaban" name="kunci_jawaban" class="form-control">
																	...
																	<option <?php echo set_select('kunci_jawaban', '5', (!isset($data->kunci_jawaban) ? FALSE : ($data->kunci_jawaban == '5' ? TRUE : FALSE)) );?> value="4">Jawaban 5</option>
																</select>
															</div>
														</div>
...
<script type="text/javascript">
	function fetch_select_materi_pokok(val)
	{
		$.ajax({
			type: 'POST',
			url: "<?=base_url('psep_sekolah/materi/ajax_select_materi_pokok')?>",
			data: { id:val },
...
<script type="text/javascript">
	...
	//   $.ajax({
	//     type: 'POST',
	//     url: "<?=base_url('psep_sekolah/materi/ajax_konten_video')?>",
	//     data: { data:url },
	//     success: function(response){
