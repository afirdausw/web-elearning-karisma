...
class Materi extends CI_Controller
{
    ...
    public function index($kelas = 0, $mapel = 0, $mapok = 0, $page = 1)
    {
        ...
        $data = array(
            ...
            'data_tabel'           => $this->Model_adm1->get_all_materi_filter($kelas, $mapel, $mapok, $offset, $limit, $carisekolah->jenjang),
        ...
        $this->load->view('psep_sekolah/materi', $data);
    }
    ...
    public function tabel_ajax($kelas = 0, $mapel = 0, $mapok = 0, $page = 1)
    {
        ...
        $this->load->view('psep_sekolah/materi_ajaxpage', $data);
    }
...
//                    ->set_output(json_encode($data));
        $this->load->view('psep_sekolah/materi_new', $data);
    }
    ...
    public function manajemen($aksi, $kelas = 0, $mapel = 0, $mapok = 0)
    {
        ...
        $jenjang_fetch = $this->Model_adm->fetch_jenjang_materi_pokok_by_kelas($kelas);
        foreach ($jenjang_fetch as $j) {
            $jenjang = $j->jenjang;
        }
        if ($aksi) {
            ...
            switch ($aksi) {
                case 'tambah':
                    ...
                    if ($cariidmapel->level == "guru") {
                        $mapel = $this->Model_psep->cari_kelas_in_id_mapel($id_mapel);
                    } else {
                        $mapel =$this->Model_adm->fetch_options_materi_pokok();
                    }
                    $data = array(
                        'navbar_title'                => "Manajemen Materi",
                        'page_title'                  => "Tambah Materi",
                        'form_action'                 => current_url(),
                        'select_options_mapel'        => $mapel,
...
//                            ->set_output(json_encode($cariidmapel, TRUE));
                    }
                    break;
