	...
	?>
		<tr>
			...
			<?php
				if($ikutagcu==1){
					?>
						<a href="<?php echo base_url("psep_sekolah/agcu/siswa/".$siswa->id_siswa);?>">Report AGCU</a>
					<?php
				}else{
					?>
...
function rekap($kelas = 0){
	redirect(base_url('psep_sekolah/agcu/report_siswa'));
	// $idpsep = $this->session->userdata('idpsepsekolah');
	...
	// if($kelas != 0){		
	// 	$idsekolah = $this->session->userdata('idsekolah');
	// 	$carisiswa = $this->model_psep->cari_siswa_by_kelas_who_do_agcutest($kelas, $idsekolah);
