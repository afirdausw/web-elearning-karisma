...
class Kelas extends CI_Controller
{
    ...
    public function proses_tambah()
    {
        ...
        if (isset($params['deskripsi_kelas'])) {
            $deskripsi_kelas = htmlspecialchars_decode(html_escape($this->input->post('deskripsi_kelas', FALSE)));
        } else {
            $deskripsi_kelas = '';
        }
        $gambar = "";
        $file_element_name = "gambar_kelas";

        if (isset($_FILES[$file_element_name]['name']) && $_FILES[$file_element_name]['name'] != "") {
            $config['upload_path'] = './image/kelas/';
    ...
    public function proses_ubah($id)
    {
        ...
        $kelas = $this->fetch_kelas_by_id($id);
        //fetch input (make sure that the variable name is the same as column name in database!)
        $params = $this->input->post(null, true);
        $jenjang = $params['jenjang'];
        $tingkatan_kelas = $params['tingkatan_kelas'];
        ...
        $file_element_name = 'gambar_kelas';  //this is the name of your input file. for example "image"
        if (isset($_FILES[$file_element_name]['name']) && $_FILES[$file_element_name]['name'] != "") {
            $config['upload_path'] = './image/kelas/';
            $config['allowed_types'] = 'gif|jpg|png|jpeg';
            $config['max_size'] = '8192';
            ...
            if (!$this->upload->do_upload($file_element_name)) {
                ...
                if ($kelas->gambar_kelas != "") {
                    unlink('image/kelas/' . $kelas->gambar_kelas);
                }
                $gambar = $data_upload['file_name'];
            }
        } else {
            $gambar = $kelas->gambar_kelas;
        }
