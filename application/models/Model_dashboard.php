...
// --
    ...
    {
        $this->db->select("`mapel`.`id_mapel`, `mapel`.`kelas_id`, `mapel`.`nama_mapel`, `mapel`.`deskripsi_mapel`, `mapel`.`gambar_mapel`, `kelas`.`alias_kelas`");
        $this->db->from("mata_pelajaran as mapel");
        $this->db->join("kelas", "mapel.kelas_id=kelas.id_kelas", "left");
        $this->db->where("`mapel`.`kelas_id`", $idkelas);
    ...
    function get_tryout_by_profil($idprofil, $idkelas)
    {
        $this->db->select("*");
        $this->db->from("kategori_tryout");
        ...
        $this->db->select('kategori_tryout.status as kategori_status');
        $query = $this->db->get();
        return $query->result();
    ...
    function analisatopik($idkategori, $idsiswa)
    {
        $this->db->select("*");
        $this->db->from("analisis_topik");
        $this->db->join("bank_soal", "analisis_topik.id_soal=bank_soal.id_banksoal","right");
        $this->db->where("analisis_topik.id_kategori", $idkategori);
        $this->db->where("analisis_topik.id_siswa", $idsiswa);
        $this->db->group_by("bank_soal.topik");
        $result = $this->db->get();
        return $result->result();
    }
    function analisistopik($idkategori, $idsiswa)
    {
        $this->db->select("*,analisis_topik.status as status_topik");
        $this->db->from("analisis_topik");
        $this->db->join("bank_soal", "analisis_topik.id_soal=bank_soal.id_banksoal","right");
        $this->db->where("analisis_topik.id_kategori", $idkategori);
        $this->db->where("analisis_topik.id_siswa", $idsiswa);
    ...
    function analisatopikbenar($idkategori, $idsiswa)
    {
        $this->db->select("*");
        $this->db->from("analisis_topik");
        $this->db->join("bank_soal", "analisis_topik.id_soal=bank_soal.id_banksoal");
        $this->db->where("analisis_topik.terjawab=bank_soal.kunci");
        $this->db->where("analisis_topik.id_kategori", $idkategori);
        $this->db->where("analisis_topik.id_siswa", $idsiswa);
        $this->db->group_by("bank_soal.topik");
    ...
    function analisabytopikbenar($idkategori,$topik, $idsiswa)
    {
        $this->db->select("*");
        $this->db->from("analisis_topik");
        $this->db->join("bank_soal", "analisis_topik.id_soal=bank_soal.id_banksoal");
    ...
    function jumlahtopik($idkategori, $idsiswa,$topik)
    {
        $this->db->select("*");
        $this->db->from("analisis_topik");
        $this->db->join("bank_soal", "analisis_topik.id_soal=bank_soal.id_banksoal");
    ...
    function kelas_by_id_in($idkelas)
    {
        $this->db->select("*");
        $this->db->from("kelas");
        $this->db->where_in("id_kelas", $idkelas);
        $query = $this->db->get();
        return $query->result();
    }
...
//update 12 november 2016 part II
    ...
    function analisis_waktu($idkategori, $idsiswa)
    {
        $this->db->select("*");
        $this->db->from("analisis_waktu");
        $this->db->where("id_kategori", $idkategori);
