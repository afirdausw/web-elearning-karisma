...
class Model_adm extends CI_Model
{
    ...
    function toogle_akses_materi_pokok($id_materi_pokok, $pretest_status=0){
        ...
        $data = array(
            'pretest_status' => $pretest_change,
        );
        $this->db->where('id_materi_pokok', $id_materi_pokok);
    ...
    function select_max_urutan($table, $where, $field)
    {
        $this->db->select_max($field);
        $this->db->where('materi_pokok_id', $where);
        $query = $this->db->get($table);
    ...
    function fetch_materi_by_id_result($id_sub_materi)
    {
        $this->db->select('*');
        $this->db->from('konten_materi');
        $this->db->join('sub_materi', 'sub_materi.id_sub_materi = konten_materi.sub_materi_id', 'left');
    ...
    // KELAS
    function fetch_all_kelas($jenjang = '')
    {
    ...
    function fetch_mapel_by_id_kelas($id_kelas)
    {
        $this->db->select('*');
        $this->db->from('mata_pelajaran');
        $this->db->join('kelas', 'kelas.id_kelas = mata_pelajaran.kelas_id');
        $this->db->where('mata_pelajaran.kelas_id', $id_kelas);
        // echo $this->db->_compile_select();
    ...
    function fetch_all_materi_pokok_by_id_mapel($id_mapel)
    {
        $this->db->select('*');
        $this->db->from('materi_pokok');
        $this->db->join('mata_pelajaran', 'mata_pelajaran.id_mapel = materi_pokok.mapel_id', 'left');
        $this->db->join('kelas', 'kelas.id_kelas = mata_pelajaran.kelas_id', 'left');
        $this->db->where('materi_pokok.mapel_id', $id_mapel);
        $this->db->order_by('materi_pokok.urutan');
        //tester
        // echo $this->db->_compile_select();
    ...
    function fetch_options_materi_pokok()
    {
        ...
        $this->db->from('mata_pelajaran');
        $this->db->join('kelas', 'kelas.id_kelas = mata_pelajaran.kelas_id', 'left');
        $this->db->order_by('mata_pelajaran.kelas_id', 'ASC');
        //tester
        // echo $this->db->_compile_select();
    ...
    function fetch_options_materi_pokok_by_kelas()
    {
        $this->db->select('*');
        $this->db->from('mata_pelajaran');
        $this->db->join('kelas', 'kelas.id_kelas = mata_pelajaran.kelas_id', 'left');
    ...
    function fetch_jenjang_materi_pokok_by_kelas($id_kelas)
    {
    ...
    function add_materi_pokok($mapel_id, $nama_materi_pokok, $pretest, $gambar_materi_pokok, $deskripsi_materi_pokok, $urutan)
    {
        ...
        $data = array(
            ...
            'gambar_materi_pokok'    => $gambar_materi_pokok,
    ...
    function update_materi_pokok($id, $mapel_id, $nama_materi_pokok, $gambar_materi_pokok, $deskripsi_materi_pokok)
    {
    ...
    //SEKOLAH
    function fetch_all_sekolah($jenjang = '')
    {
        $this->db->select('*');
        ...
        if ($jenjang != '') {
            $this->db->where('sekolah.jenjang', $jenjang);
        }
        $query = $this->db->get();
    ...
    function import_siswa($data_sekolah)
    {
        //Insert data into table sekolah
        foreach ($data_sekolah as $data) {
            $data2 = array(
                'username' => ($data['nisn'] != '' ? $data['nisn'] : $data['nisn']),
                'password' => md5(($data['nisn'] != '' ? $data['nisn'] : $data['nisn'])),
                'role'     => 'siswa',
            );
            $result = $this->db->insert('login_siswa', $data2);
            $insert_id = $this->db->insert_id();
            $data['id_login'] = $insert_id;
    ...
    function import_latihan_soal($data_sekolah)
    {
        //Insert data into table sekolah
        foreach ($data_sekolah as $data) {
            ...
            $result = $this->db->insert('soal', $data3);
            ...
            if ($this->insert_id != NULL) {
                ...
                $data2 = array(
                    ...
                    'pembahasan'       => '',
                    'pembahasan_video' => '',
                    'jawab_1'          => $data['jawab_1'],
                    'jawab_2'          => $data['jawab_2'],
                    'jawab_3'          => $data['jawab_3'],
                    'jawab_4'          => $data['jawab_4'],
                    'jawab_5'          => $data['jawab_5'],
                    'kunci_jawaban'    => $data['kunci_jawaban'],
                );
                $result = $this->db->insert('jawaban', $data2);
            }
        }
    ...
    function import_bank_soal($data_sekolah)
    {
        //Insert data into table sekolah
        foreach ($data_sekolah as $data) {
    ...
    function add_siswa($nama, $email, $telepon, $telepon_ortu, $alamat, $sekolah_id, $kelas, $nisn, $nis, $username = '', $password = '')
    {
        //Insert data into table mata_pelajaran
        $data = array(
            'username' => ($username != '' ? $username : $nisn),
            'password' => md5(($password != '' ? $password : $nisn)),
            'role'     => 'siswa',
        );
    ...
    function update_siswa($id, $nama, $email, $telepon, $telepon_ortu, $alamat, $sekolah_id, $kelas, $nis, $nisn)
    {
        //Update row by id in table mata_pelajaran
        $data = array(
            'nama_siswa'   => $nama,
    ...
    {
        $siswa = $this->fetch_siswa_by_id($id);
    ...
    // LOG AKSES
    function fetch_log_siswa($jenjang = '')
    {
    ...
    function fetch_all_soal()
    {
        ...
        $this->db->where('sub_materi.id_sub_materi !=', "null");
        //	$this->db->where('login_sekolah.id_mapel', "20");
        //tester
    ...
    function fetch_all_soal_by_jenjang($jenjang)
    {
        $this->db->select('*');
        $this->db->from('soal');
        $this->db->join('sub_materi', 'soal.sub_materi_id = sub_materi.id_sub_materi', 'left');
    ...
    // LATIHAN SOAL	(IFA)
    function fetch_all_soal_by_mapel($id_mapel)
    {
        $this->db->select('*');
        $this->db->from('soal');
        $this->db->join('sub_materi', 'soal.sub_materi_id = sub_materi.id_sub_materi', 'left');
    ...
    function fetch_soal_by_submateri($id_sub_materi, $jenis_fetch = "")
    {
        $this->db->select('*');
    ...
    function add_item_soal_bank_soal($id, $isi_soal, $jawab_1, $jawab_2, $jawab_3, $jawab_4, $jawab_5, $kunci_jawaban, $sub_materi_id, $pembahasan, $pembahasan_video)
    {
        //Insert data into table soal
        $data = array(
            'isi_soal'      => $isi_soal,
    ...
    function select_item_soal_bank_soal($id_bank_soal, $sub_materi_id)
    {
        $this->db->where('id_banksoal', $id_bank_soal);
        $this->db->where('sub_materi_id', $sub_materi_id);
    ...
    function select_item($sub_materi_id)
    {
        $this->db->where('id_banksoal <> ', 0);
        $this->db->where('sub_materi_id', $sub_materi_id);
        $result = $this->db->get('soal');
    ...
    function fetch_all_profil_by_jenjang($jenjang)
    {
    ...
    function fetch_all_profil_by_jenjang_limit($jumdatalimit, $page, $jenjang)
    {
        ...
        $this->db->join("kelas", "kelas.id_kelas=profil_tryout.id_kelas");
        $this->db->where("kelas.jenjang", $jenjang);
        $this->db->limit($jumdatalimit, $page);
    ...
    function fetch_all_profil_limit($jumdatalimit, $page)
    {
        $this->db->select("
		profil_tryout.id_tryout,
		profil_tryout.id_kelas,
    ...
    function fetch_all_profil_by_kelas($kelas)
    {
        ...
        $this->db->from("profil_tryout");
        $this->db->join("kelas", "kelas.id_kelas=profil_tryout.id_kelas");
        $this->db->where("profil_tryout.id_kelas", $kelas);
    ...
    function fetch_all_profil_by_id($id_profil)
    {
        ...
        $this->db->from("profil_tryout");
        $this->db->join("kelas", "kelas.id_kelas=profil_tryout.id_kelas");
        $this->db->where("profil_tryout.id_tryout", $id_profil);
                         ...
                         $penyelenggara, $tgl, $jam, $biaya, $banner, $tanggalpost, $waktupost, $tipe)
    {
        ...
        $result = $this->db->update('profil_tryout', $profil);
    ...
    function add_kategori($idprofil, $nama, $random, $tanggal, $jam, $waktu, $ketuntasan, $jumlah, $mata_pelajaran)
    {
        $data = array(
            'id_profil'     => $idprofil,
            'nama_kategori' => $nama,
    ...
    function last_addedkategori_row()
    {
        $this->db->select("max(id_kategori) as id_terakhir");
        $this->db->from("kategori_tryout");
        $result = $this->db->get();
    ...
    function fetch_soaltryout_by_kategori($idkategori = '')
    {
        $this->db->select('*');
    ...
    function updatejumlah_soaltryout_kategori($idkategori, $jml)
    {
        $data = array(
            'jumlah_soal' => $jml,
        );
    ...
    /*STOP PAKET*/
    function add_soal_kategori($idkategori, $idbanksoal)
    {
        $data = array(
            'id_kategori' => $idkategori,
            'id_banksoal' => $idbanksoal,
    ...
    function delete_soal_kategori($idkategori, $idbanksoal)
    {
        $data = array(
            'id_kategori' => $idkategori,
            'id_banksoal' => $idbanksoal,
        ...
        $this->db->where($data);
        $result = $this->db->delete('soal_tryout');
    ...
    function fetch_kategori_byid($id_kategori)
    {
        $this->db->select("*");
        $this->db->from("kategori_tryout");
    ...
    function fetch_profiledit($id_tryout)
    {
        $this->db->select("*");
        $this->db->from("profil_tryout");
    ...
    {
        $this->db->delete('profil_tryout', array('id_tryout' => $idprofil));
    }
